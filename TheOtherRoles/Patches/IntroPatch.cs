            ...
            static public void SetRoleTexts(IntroCutscene __instance)
            {
                ...
                if (Deputy.knowsSheriff && Deputy.exists && Sheriff.exists)
                {
                    if (infos.Any(info => info.roleId == RoleId.Sheriff))
                    {
                        ...
                        if (deputy != null) __instance.RoleBlurbText.text += Helpers.cs(Sheriff.color, $"\nYour Deputy is {deputy?.player?.Data?.PlayerName ?? ""}");
                    }
                    else if (infos.Any(info => info.roleId == RoleId.Deputy))
                        __instance.RoleBlurbText.text += Helpers.cs(Sheriff.color, $"\nYour Sheriff is {Deputy.getRole(PlayerControl.LocalPlayer)?.sheriff?.player?.Data?.PlayerName ?? ""}");
                }
            }
