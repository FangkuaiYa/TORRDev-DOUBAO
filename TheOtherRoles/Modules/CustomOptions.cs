...
namespace TheOtherRoles
{
    public class CustomOption
    {
        ...
        public CustomOption(int id, string stringId, CustomOptionType type, string name, System.Object[] selections, System.Object defaultValue, CustomOption parent, bool isHeader, Action onChange = null, string heading = "", bool invertedParent = false, Color headerBGColor = new Color())
        {
            this.id = id;
        ...
        public static CustomOption CreateRoleOption(CustomOptionType type, Color color, string name, string[] selections, CustomOption parent = null, bool isHeader = false, Action onChange = null, string heading = "", bool invertedParent = false, string stringId = "")
        {
            return new CustomOption(idI++, stringId, type, name, selections, "", parent, isHeader, onChange, heading, invertedParent);
        }
        ...
        {
            return new CustomOption(idI++, stringId, type, name, new string[] { "Off", "On" }, defaultValue ? "On" : "Off", parent, isHeader, onChange, heading, invertedParent);
        }
        ...
        public static void drawTab(LobbyViewSettingsPane __instance, CustomOptionType optionType)
        {
            ...
            foreach (var option in relevantOptions)
            {
                if (option.isHeader && (int)optionType != 99 || (int)optionType == 99 && curType != option.type)
                {
                    ...
                    if (option.headerBGColor != new Color())
                        categoryHeaderMasked.Background.color = option.headerBGColor;
                    if ((int)optionType == 99)
                ...
                else if (option.parent != null && (option.parent.selection == 0 || option.parent.parent != null && (option.parent.parent.selection == 0 || option.parent.parent.parent != null && option.parent.parent.parent.selection == 0))) continue;  // Hides options, for which the parent is disabled!
                ...
                if (option.headerBGColor != new Color())
                    viewSettingsInfoPanel.background.color = option.headerBGColor;
