...
internal static class HudManagerStartPatch
{
    ...
    public static void createButtonsPostfix(HudManager __instance)
    {
        ...
        sheriffKillButton = new CustomButton(
            () =>
            {
                ...
                if (murderAttemptResult == MurderAttemptResult.PerformKill)
                {
                    ...
                    if ((Sheriff.local.currentTarget.Data.Role.IsImpostor && (Sheriff.local.currentTarget != Mini.mini || Mini.isGrownUp())) ||
                        (Sheriff.spyCanDieToSheriff && Spy.spy == Sheriff.local.currentTarget) ||
                        (Sheriff.canKillNeutrals && Helpers.isNeutral(Sheriff.local.currentTarget)) ||
                        (Jackal.jackal == Sheriff.local.currentTarget || Sidekick.sidekick == Sheriff.local.currentTarget))
                    {
                    ...
                    if (targetId == Sheriff.local.player.PlayerId && Helpers.checkArmored(Sheriff.local.player, true, true))
                        return;
            ...
            () => { return Sheriff.local.player != null && Sheriff.local.player == PlayerControl.LocalPlayer && !PlayerControl.LocalPlayer.Data.IsDead; },
        ...
        morphlingButton = new CustomButton(
            () =>
            {
                if (Morphling.sampledTarget != null)
                {
                    ...
                    morphlingButton.buttonText = ModTranslation.getString("morphlingSample");
                }
                else if (Morphling.currentTarget != null)
                {
                    ...
                    morphlingButton.buttonText = ModTranslation.getString("morphlingMorph");
        ...
        portalmakerPlacePortalButton = new CustomButton(
            () =>
            {
                ...
                var writer =
                    AmongUsClient.Instance.StartRpcImmediately(PlayerControl.LocalPlayer.NetId, (byte)CustomRPC.PlacePortal, Hazel.SendOption.Reliable);
                writer.WriteBytesAndSize(buff);
                writer.EndMessage();
        ...
        warlockCurseButton = new CustomButton(
            () =>
            {
                ...
                else if (Warlock.curseVictim != null && Warlock.curseVictimTarget != null)
                {
                    ...
                    warlockCurseButton.buttonText = ModTranslation.getString("warlockCurse");
        ...
        securityGuardButton = new CustomButton(
            () =>
            {
                if (SecurityGuard.ventTarget != null)
                {
                    // Seal vent
                    var writer =
                        AmongUsClient.Instance.StartRpcImmediately(PlayerControl.LocalPlayer.NetId, (byte)CustomRPC.SealVent, Hazel.SendOption.Reliable);
                ...
                else if (!Helpers.isMira() && !Helpers.isFungle() && !SubmergedCompatibility.IsSubmerged)
                {
                    ...
                    var writer =
                        AmongUsClient.Instance.StartRpcImmediately(PlayerControl.LocalPlayer.NetId, (byte)CustomRPC.PlaceCamera, Hazel.SendOption.Reliable);
                    writer.WriteBytesAndSize(buff);
                    writer.EndMessage();
            ...
            () =>
            {
                ...
                if (SecurityGuard.ventTarget == null && !Helpers.isMira() && !Helpers.isFungle() &&
                    ...
                    !SubmergedCompatibility.IsSubmerged)
                    securityGuardButton.buttonText = ModTranslation.getString("securityGuardPlaceCam");
                else
                    securityGuardButton.buttonText = ModTranslation.getString("securityGuardSealVent");
        ...
        arsonistButton = new CustomButton(
            ...
            () =>
            {
                ...
                if (dousedEveryoneAlive)
                {
                    ...
                    arsonistButton.buttonText = ModTranslation.getString("arsonistIgnite");
                }
        ...
        ninjaButton = new CustomButton(
            () =>
            {
                ...
                {
                    ninjaButton.buttonText = ModTranslation.getString("ninjaMarkButton");
                    ...
                    if (attempt == MurderAttemptResult.PerformKill)
                    {
                        ...
                        writer = AmongUsClient.Instance.StartRpcImmediately(PlayerControl.LocalPlayer.NetId,
                            (byte)CustomRPC.PlaceNinjaTrace, Hazel.SendOption.Reliable);
                        writer.WriteBytesAndSize(buff);
                        writer.EndMessage();
                ...
                {
                    ninjaButton.buttonText = ModTranslation.getString("ninjaAssassinateButton");
        ...
        mayorMeetingButton = new CustomButton(
            ...
            () =>
            {
                mayorMeetingButton.actionButton.OverrideText(ModTranslation.getString("mayorMeetingButton") + " (" +
                                                             Mayor.remoteMeetingsLeft + ")");
                var sabotageActive = false;
                foreach (var task in PlayerControl.LocalPlayer.myTasks.GetFastEnumerator())
                    if (task.TaskType == TaskTypes.FixLights || task.TaskType == TaskTypes.RestoreOxy ||
        ...
        trapperButton = new CustomButton(
            () =>
            {
                ...
                var writer = AmongUsClient.Instance.StartRpcImmediately(PlayerControl.LocalPlayer.NetId, (byte)CustomRPC.SetTrap, Hazel.SendOption.Reliable);
                writer.WriteBytesAndSize(buff);
                writer.EndMessage();
        ...
        bomberButton = new CustomButton(
            () =>
            {
                    ...
                    MurderAttemptResult.BlankKill)
                {
                    ...
                    var writer =
                        AmongUsClient.Instance.StartRpcImmediately(PlayerControl.LocalPlayer.NetId, (byte)CustomRPC.PlaceBomb, Hazel.SendOption.Reliable);
                    writer.WriteBytesAndSize(buff);
                    writer.EndMessage();
        ...
        yoyoButton = new CustomButton(
            () =>
            {
                ...
                {
                    var writer = AmongUsClient.Instance.StartRpcImmediately(PlayerControl.LocalPlayer.NetId,
                        (byte)CustomRPC.YoyoMarkLocation, Hazel.SendOption.Reliable);
                    writer.WriteBytesAndSize(buff);
                    writer.EndMessage();
                    ...
                    yoyoButton.buttonText = ModTranslation.getString("yoyoButtonBlink");
                }
                else
                {
                    // Jump to location
                    ...
                    if (SubmergedCompatibility.IsSubmerged) SubmergedCompatibility.ChangeFloor(exit.y > -7);
                    var writer =
                        AmongUsClient.Instance.StartRpcImmediately(PlayerControl.LocalPlayer.NetId, (byte)CustomRPC.YoyoBlink, Hazel.SendOption.Reliable);
                    writer.Write(byte.MaxValue);
                    writer.WriteBytesAndSize(buff);
                    writer.EndMessage();
                    ...
                    yoyoButton.buttonText = ModTranslation.getString("yoyoButtonBlinkReturning");
                    SoundEffectsManager.play("morphlingMorph");
                }
            },
            () =>
            ...
            () =>
            {
                ...
                else
                {
                    ...
                    yoyoButton.buttonText = ModTranslation.getString("yoyoButtonMark");
                }
            },
